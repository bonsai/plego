...
|---------|------------|------|--------|
| **v0** (現在アクティブ) | `plego.v0.yaml` | iCal フィードのみ | 不要 |
| v1 | `plego.v1.yaml` | iCal + 日次メール | `GMAIL_USER` `GMAIL_APP_PASSWORD` |
| v2 | `plego.v2.yaml` | iCal + メール + Google Calendar API | + `GOOGLE_CALENDAR_CREDENTIALS` `GOOGLE_CALENDAR_ID` |

`plego.yaml` は現在 v0 と同内容。バージョンアップ時は `cp plego.vX.yaml plego.yaml` して commit。

## アーキテクチャ

```
cmd/plego/main.go          エントリポイント、プラグインをワイヤリング
...
3. `cmd/plego/main.go` の `buildSource()` に case 追加
...
3. `cmd/plego/main.go` の `buildOutput()` に case 追加

## GitHub Actions

| ワークフロー | スケジュール | 用途 |
|------------|----------|------|
| `digest-v0.yml` | 毎日 JST 07:00 | iCal 深咔 (アクティブ) |
...
- キーワード・業種フィルタの上書き: `plego.yaml` を編集するか、将来的に設定 UI を追加。
